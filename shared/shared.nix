  ...
  shared.wg = {
    peers = {
      algedi = {
        ...
        allowedIPs =
          [ "${config.shared.wg.all4}2/32" "${config.shared.wg.all6}2/128" ];
      ...
      mintaka = {
        ...
        allowedIPs =
          [ "${config.shared.wg.all4}4/32" "${config.shared.wg.all6}4/128" ];
      };
      polaris = {
        publicKey = builtins.replaceStrings [ "\n" ] [ "" ]
          (builtins.readFile ./wg_keys/polaris);
        allowedIPs =
          [ "${config.shared.wg.all4}3/32" "${config.shared.wg.all6}3/128" ];
      };
      proxima = {
        publicKey = builtins.replaceStrings [ "\n" ] [ "" ]
          (builtins.readFile ./wg_keys/proxima);
        allowedIPs =
          [ "${config.shared.wg.all4}0/24" "${config.shared.wg.all6}0/64" ];
      ...
      elnath = {
        ...
        allowedIPs =
          [ "${config.shared.wg.all4}8/32" "${config.shared.wg.all6}8/128" ];
